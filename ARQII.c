...
int main(int argc,char **argv)
{
	...
	int count=0, count_aux=0,total_count=0,no_div=0, no_div_aux=0,fin_no = 0 , x;
	MPI_Status st;
	MPI_Init(NULL,NULL);
	...
	if(rank ==0)
		{
			for(x=1; x < size; x++){
				...
				MPI_Recv(&count_aux,1,MPI_INIT,0,0,MPI_COMM_WORLD,&st);
				count = count + count_aux;
				MPI_Recv(&no_div_aux,1,MPI_INIT,0,1,MPI_COMM_WORLD,&st);
